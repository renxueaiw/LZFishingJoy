...
bool StartLayer::init()
{
    if (CCLayer::init()) {
        ...
        title->setPosition(CCPointMake(winSize.width*0.5f, winSize.height*0.7f));
        this->addChild(title);
        ...
        if (CCDirector::sharedDirector()->getTotalFrames() < 10) {
            ...
            _progressBar->setPosition(CCPointMake(winSize.width*0.5f, winSize.height*0.5f));
...
void StartLayer::initializationCompleted()
{
    ...
    menu->setPosition(CCPointMake(winSize.width*0.5f, winSize.height*0.4f));
